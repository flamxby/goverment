...
def get(reservation_id: int, db: Session):
    ...
    if not reservation.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail="No reservation with this id")
    else:
...
def get_from_specific_date(selected_year: int, selected_month: int, selected_day: int, db: Session):
    ...
    max_datetime = datetime(selected_year, selected_month,
                            selected_day, 23, 59, 59, 999999)
    reservation = db.query(models.Reservation).filter(
        models.Reservation.register_timestamp.between(min_datetime, max_datetime)).all()
    return reservation
...
def get_all_new(db: Session):
    all_new_reservations = db.query(models.Reservation).filter(
        func.DATE(models.Reservation.register_timestamp) == date.today()).all()
